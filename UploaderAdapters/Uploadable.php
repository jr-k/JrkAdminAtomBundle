...
trait Uploadable
{
    ...
    public function getUploadDirectoryPath($create = false) {
        $dir = 'uploads/'.basename(str_replace('\\','/',strtolower(__CLASS__))).'-'.substr(md5(__NAMESPACE__),0,6).'/'.implode('/',str_split($this->getId()));
    ...
    public function getResourceDirectoryPath() {
        return 'resources/'.strtolower(__CLASS__).'-'.substr(md5(__NAMESPACE__),0,6);
    }
